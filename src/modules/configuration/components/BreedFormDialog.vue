  ...
  <v-dialog v-model="dialog" max-width="460" persistent>
    <v-card>
      <v-card-title class="d-flex align-center pt-4 px-6">
        <v-icon color="primary" class="mr-2">{{ isEdit ? 'mdi-pencil-outline' : 'mdi-dna' }}</v-icon>
...
export default {
  ...
  methods: {
    ...
    // Called from the parent via ref: open() to create, open(breed) to edit
    open(breed = null) {
      this.editId = breed ? breed.id : null
      this.form = { name: breed ? breed.name : '' }
      this.error = ''
      this.dialog = true
    },
    close() {
      ...
      try {
        ...
        if (this.isEdit) {
          await this.updateItem({
            ...
            nameState: 'breeds',
            url: `/configuration/breeds/${this.editId}/`,
            data: payload,
          })
        } else {
        ...
        this.error = getErrorMessage(e, 'No se pudo guardar la raza')
      } finally {
        this.saving = false
      }
    },
